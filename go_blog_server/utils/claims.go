...
func SetRefreshToken(c *gin.Context, token string, maxAge int) {
	...
	host, _, err := net.SplitHostPort(c.Request.Host)  // 返回请求的 host port 和 err
	if err != nil {
		host = c.Request.Host
	}
...
func setCookie(c *gin.Context, name, value string, maxAge int, host string) {
	...
	if net.ParseIP(host) != nil {
		...
		c.SetCookie(name, value, maxAge, "/", "", false, true)	// 倒数第二个参数secure： true / false：仅HTTPS传输 / HTTP和HTTPS都可以传输
	} else {
		// 如果是域名，设置cookie的domain为域名
		c.SetCookie(name, value, maxAge, "/", host, false, true)	// // 倒数第二个参数httpOnly： true / false：JavaScript无法访问，防止XSS攻击 / 可通过JS访问
	}
}
