...
func MySQLPagination[T any](model *T, option other.MySQLOption) (list []T, total int64, err error) { // 泛型参数 T 表示要查询的模型类型
	// 设置分页的默认值
	if option.Page < 1 {
...
func EsPagination(ctx context.Context, option other.EsOption) (list []types.Hit, total int64, err error) {
	...
	res, err := global.ESClient.Search().
		...
		SourceIncludes_(option.SourceIncludes...). // 设置需要包含的字段	option.SourceIncludes...：把string切片拆分成一个个string
		Do(ctx)                                    // 执行查询
	if err != nil {
