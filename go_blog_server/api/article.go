...
// 置顶文章
func (ArticleApi *ArticleApi) ArticleTop(c *gin.Context) {
	var articleTop request.ArticleTop
	err := c.ShouldBindJSON(&articleTop)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}

	err = articleService.ArticleTop(articleTop)
	if err != nil {
		global.Log.Error("Failed to top article:", zap.Error(err))
		response.FailWithMessage("Failed to top article", c)
		return
	}
	response.OkWithMessage("Successfully toggled article top status", c)
}
