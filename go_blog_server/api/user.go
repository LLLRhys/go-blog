...
func (userApi *UserApi) Register(c *gin.Context) {
	...
	if savedEmail == nil || savedEmail.(string) != req.Email {  // session里存了邮箱(发送过邮箱验证码才存)，同时session存的邮箱和请求邮箱一致 才继续
...
func (userApi *UserApi) TokenNext(c *gin.Context, user database.User) {
	...
	j := utils.NewJWT()  // 创建 JWT 实例,包含access和refresh密钥

	// 创建访问令牌
	accessClaims := j.CreateAccessClaims(baseClaims)	// 创建访问声明
	...
	// 创建刷新令牌
	refreshClaims := j.CreateRefreshClaims(baseClaims)  	// 创建刷新声明
	...
	// 检查 Redis 中是否已存在该用户的 RefreshToken
	if jwtStr, err := jwtService.GetRedisJWT(user.UUID); errors.Is(err, redis.Nil) {	// errors.Is()判断两个错误是否相等；redis.Nil表示键值对不存在
		// 不存在就设置新的
		...
		response.OkWithDetailed(response.Login{
			User:                 user,
			AccessToken:          accessToken,
			AccessTokenExpiresAt: accessClaims.ExpiresAt.Unix() * 1000,	// Go中的Unix是秒级毫秒，Js中的Unix是毫秒级秒 倍率为1000
		}, "Successful login", c)
	} else if err != nil {	
		// redis操作出错
		global.Log.Error("Failed to set login status:", zap.Error(err))
...
func (userApi *UserApi) ForgotPassword(c *gin.Context) {
	...
	// 两次邮箱一致性判断
	savedEmail := session.Get("email")	// session里面的email和code都是发送验证码时存进去的
	if savedEmail == nil || savedEmail.(string) != req.Email {	// savedEamil不为nil，表示服务器发送了验证码
		response.FailWithMessage("This email doesn't match the email to be verified", c)
		return
	}
...
func (userApi *UserApi) UserChangeInfo(c *gin.Context) {
	...
	req.UserID = utils.GetUserID(c)	// 接受请求体中UserID的json标签为 - ，不接受值。这里要手动获取UserID(UserID必须是登陆这自己的)
