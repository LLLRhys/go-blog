...
func (articleService *ArticleService) Exits(title string) (bool, error) {
	...
	// 执行搜索查询，查找是否存在该标题的文章
	res, err := global.ESClient.Search().Index(elasticsearch.ArticleIndex()).Request(req).Size(1).Do(context.TODO())	// 链式调用
	if err != nil {
		return false, err
	}
	// 如果存在该标题，返回 true
...
//	一个方法三种情形：1.增时，old空 new有；2.改时，old有 new有；3.删时，old有 new无
