...
// JsonInBlacklist 将RefreshToken添加到黑名单
func (jwtService *JwtService) JoinInBlacklist(jwtList database.JwtBlacklist) error {
	...
	// 将RefreshToken添加到内存中的黑名单缓存，缓存的过期时间与RefreshToken的过期时间一致（在initialize/other.go中有设置）
	global.BlackCache.SetDefault(jwtList.Jwt, struct{}{})	// 使用空结构体作为值 节省空间，因为只关心键的存在与否
	return nil
}

// IsBlacklist 检查RefreshToken是否在黑名单中
func (jwtService *JwtService) IsInBlacklist(jwt string) bool {
	// 从黑名单缓存中检查RefreshToken是否存在
