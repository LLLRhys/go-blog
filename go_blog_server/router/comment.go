...
func (c *CommentRouter) InitCommentRouter(Router *gin.RouterGroup, PublicRouter *gin.RouterGroup, AdminRouter *gin.RouterGroup) {
	...
	{
		commentRouter.POST("create", commentApi.CommentCreate)	// 创建后有钩子函数，model/database/comment.go
		commentRouter.DELETE("delete", commentApi.CommentDelete)	// // 删除后有钩子函数，model/database/comment.go
