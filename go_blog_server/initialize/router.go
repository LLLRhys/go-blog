...
func InitRouter() *gin.Engine {
	...
	Router.Use(sessions.Sessions("session", store))	// 使用cookie存储会话数据
	...
	// 创建路由组
	// RouterGroupApp 是一个结构体，不是路由组，而是打包了各个模块的路由注册方法的集合
	routerGroup := router.RouterGroupApp  // RouterGroupApp 起别名 routerGroup。
