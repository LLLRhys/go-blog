    ...
    >
      ...
      <el-table-column label="操作" width="160">
        <template #default="scope:{ row: any, column: any, $index: number }">
          ...
          <el-button
            :type="scope.row._source.top ? 'primary' : 'info'"
            size="small"
            style="margin-left:8px;"
            @click="handleToggleTop(scope.row)"
          >
            {{ scope.row._source.top ? '取消置顶' : '置顶' }}
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
// 切换置顶状态（无需刷新表格）
const handleToggleTop = async (row: Hit<Article>) => {
  const res = await articleTop({ article_id: row._id })
  if (res.code === 0) {
    row._source.top = !row._source.top
    ElMessage.success(res.msg || (row._source.top ? '已置顶' : '已取消置顶'))
    // 可选：如果有 top_at 字段也可同步更新
    if (row._source.top_at !== undefined && res.data && res.data.top_at) {
      row._source.top_at = res.data.top_at
    }
  } else {
